...
//     std::cout<<"shape3:"<<input->shape()[3]<<"shape2:"<<input->shape()[2]<< "shape1:"<< input->shape()[1]<<"shape0:"<<input->shape()[0]<<std::endl;
...
//     if(type == DataType::TYPE_FP32){
//         //scale_array((float*)input->data(), input_shape, (float)params.scale);
...
#pragma omp parallel for num_threads(std::min((int)(input->shape()[0] *input->shape()[1]),(int)numThreads)) if((input->shape()[0] *input->shape()[1])>=4) collapse(2)
        for(int i = 0;i<input->shape()[0];i++){
            ...
            //context_mask_float((float*)input->data() , (float*)params.mask.value().get()->data() , input->shape()[2]*input->shape()[3]);
            //context_mask<float>(params.input, params.mask.value().get());
        } 
        else {
            throw std::runtime_error("Softmax data type is not supported");
